      ...
      {parts.map((part, index) => {
        ...
        return lines.map((line, lIdx) => {
          if (line.startsWith("#")) {
            ...
            const sizeClass = level === 1 ? "text-2xl font-bold mt-6 mb-3 text-white" : "text-xl font-semibold mt-5 mb-2.5 text-white";
            return (
              <div key={`${index}-${lIdx}`} className={`border-b border-[#1f521f]/10 pb-1.5 ${sizeClass}`}>
          ...
          if (line.trim().startsWith("- ") || line.trim().startsWith("* ")) {
            ...
            return (
              <div key={`${index}-${lIdx}`} className="flex items-start gap-2.5 my-2.5 pl-4">
                <span className="text-[#33ff00] font-bold select-none font-sans">•</span>
                <span className="flex-1 text-[#e3e3e3]/90">{parseInlineStyles(text)}</span>
              </div>
            );
          }
          ...
          if (line.trim().match(/^\d+\.\s/)) {
            ...
            return (
              <div key={`${index}-${lIdx}`} className="flex items-start gap-2.5 my-2.5 pl-4">
                <span className="text-[#33ff00] font-bold select-none">{num}.</span>
                <span className="flex-1 text-[#e3e3e3]/90">{parseInlineStyles(text)}</span>
              </div>
            );
          }

          if (line.trim() === "") return <div key={`${index}-${lIdx}`} className="h-4.5" />;

          return (
            <p key={`${index}-${lIdx}`} className="my-3.5 leading-loose text-[#e3e3e3]/95">
