...
export const WorkspaceHeader: React.FC<WorkspaceHeaderProps> = memo(
  ({
    ...
    return (
      <div className="w-full h-14 flex items-center px-20 justify-between select-none relative z-20">
        <div className="flex items-center gap-3">
          <span className="text-[14px] uppercase tracking-widest text-[#33ff00]/60 font-bold font-mono">
            CURRENT CODEBASE:
          </span>
          <div className="relative">
            ...
            >
              {completedRepos.length === 0 ? (
                <option className="bg-[#131314] text-[#33ff00]">
                  No completed codebases
                </option>
              ) : (
                completedRepos.map((repo) => (
                  <option
                    key={repo.id}
                    value={repo.id}
                    className="bg-[#131314] text-[#33ff00]"
                  >
                    {repo.name}
                  </option>
                ))
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 select-none">
            ...
            <span className="text-[10px] text-[#33ff00]/60 uppercase tracking-widest font-bold font-mono">
              READY
            </span>
          </div>
        </div>
